...
public class ManageOrderProductIntegrationTest {
    ...
    @Test
    void manageOrderProduct_userNotAuthenticated_shouldRedirect() throws Exception {
        try (MockedStatic<AuthUtil> mockedAuth = mockStatic(AuthUtil.class)) {
            ...
            mockMvc.perform(get("/manageOrderProduct/1").sessionAttr("loggedInUser", new User()))
    ...
    void manageOrderProduct_notOwner_shouldRedirectToAccessDenied() throws Exception {
        ...
        try (MockedStatic<AuthUtil> mockedAuth = mockStatic(AuthUtil.class)) {
            ...
            mockMvc.perform(get("/manageOrderProduct/1")
    ...
    @Test
    void manageOrderProduct_validAccess_shouldReturnView() throws Exception {
        try (MockedStatic<AuthUtil> mockedAuth = mockStatic(AuthUtil.class)) {
            mockedAuth.when(() -> AuthUtil.checkFarmerAuth(any())).thenReturn(null);
            ...
            mockMvc.perform(get("/manageOrderProduct/1")
                    ...
                    .andExpect(view().name(ViewPaths.MANAGE_ORDER_PRODUCT))
